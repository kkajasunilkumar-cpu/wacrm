...
interface CampaignStatus { running: boolean; total: number; sent: number; failed: number; pending: number; log: CampaignLog[]; }
...
const PREV_CAMPAIGN_KEY = 'gmail_prev_campaign';
const TOTAL_STATS_KEY = 'gmail_total_stats';
...
export default function GmailCampaignPage() {
  ...
  const [campaign, setCampaign] = useState<CampaignStatus>({ running: false, total: 0, sent: 0, failed: 0, pending: 0, log: [] });
  ...
  const resetTotalStats = () => {
    if (!confirm('Reset all total statistics?')) return;
  ...
  const insertVar = (v: string) => {
    ...
    setBody(body.slice(0, start) + v + body.slice(ta.selectionEnd));
  ...
  const startCampaign = async () => {
    ...
    if (!emailContacts.length) { alert('No email column found. Make sure column is named "email".'); return; }
    ...
    try {
      const res = await fetch(`${MAILER_URL}/send-campaign`, {
        ...
        body: JSON.stringify({ gmail: settings, contacts: emailContacts, subject, body, scheduleAt: scheduleType === 'later' ? scheduleTime : null }),
      });
      ...
      setIsStarting(false);
      alert('Cannot reach mailer service.');
    }
  };
  ...
  const allVars = headers.length ? [...new Set([...headers.filter(h => defaultVars.includes(h)), ...extraVars])] : defaultVars;
    ...
    <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto">
      ...
      {serviceError && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-400">
          ⚠️ Mailer service not reachable. Run <code className="bg-zinc-800 px-2 py-0.5 rounded text-xs">pm2 start mailer-service</code> on server.
        </div>
      )}
      ...
      {totalStats.campaigns > 0 && (
        <div className="rounded-2xl border border-zinc-700 bg-gradient-to-r from-zinc-900 to-zinc-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm font-bold text-white">📈 Total Email Stats</p>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { label: 'Total Emails', value: totalStats.totalMessages, color: 'text-white', bg: 'bg-zinc-800 border-zinc-700', icon: '📨' },
              { label: 'Campaigns', value: totalStats.campaigns, color: 'text-orange-400', bg: 'bg-orange-500/10 border-orange-500/20', icon: '🚀' },
          ...
          {totalStats.totalMessages > 0 && (
            <div className="mt-4">
              ...
              <div className="h-2 rounded-full bg-zinc-700 overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-red-500 to-orange-500 transition-all duration-500"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 items-start">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            ...
            <p className="text-xs text-zinc-600 mb-4">Gmail → My Account → Security → 2FA → App Passwords</p>
            ...
            {settingsSaved && <button onClick={clearSettings} className="w-full rounded-lg border border-zinc-700 text-zinc-400 text-sm py-2 hover:border-zinc-500 transition-colors">🗑️ Clear</button>}
          </div>

          {/* Contacts */}
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-4">👥 Import Contacts</p>
            <div className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors mb-4 ${isDragging ? 'border-red-500 bg-red-500/10' : 'border-zinc-700 hover:border-zinc-500'}`}
              ...
              <p className="text-xs text-zinc-500">Required: <span className="text-red-400">email</span> column</p>
            </div>
            {contacts.length > 0 && (
              <div>
                ...
                <div className="rounded-lg border border-zinc-800 overflow-hidden max-h-44 overflow-y-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody>{contacts.slice(0, 20).map((c, i) => <tr key={i} className="border-t border-zinc-800">{headers.slice(0, 3).map((h) => <td key={h} className="px-3 py-2 text-zinc-300">{c[h]}</td>)}</tr>)}</tbody>
                  </table>
                </div>
              </div>
            )}
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={showPreview} className="text-xs text-red-400 hover:text-red-300 font-semibold">👁️ Preview</button>
            </div>
            {preview && (
            ...
            <button onClick={startCampaign} disabled={isRunning}
              ...
              {isStarting ? '⏳ Starting...' : campaign.running ? '📧 Sending...' : scheduleType === 'later' ? '⏰ Schedule Campaign' : '🚀 Send Campaign Now'}
            </button>
          </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              {campaign.running && <span className="text-xs text-red-400 animate-pulse font-semibold">🔴 Live</span>}
            ...
            <div className="h-2 rounded-full bg-zinc-800 overflow-hidden mb-2">
              <div className="h-full rounded-full bg-gradient-to-r from-red-500 to-orange-500 transition-all duration-500" style={{ width: `${isStarting ? 2 : pct}%` }} />
            </div>
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-950 max-h-64 overflow-y-auto">
              {campaign.log.length === 0 ? (
                <div className="text-center py-8 text-zinc-600 text-sm">{isStarting ? '⏳ Waiting for first email...' : 'No emails sent yet'}</div>
              ) : (
